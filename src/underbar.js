(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(truthVal, item) {
      if (iterator(item) && truthVal === true) {
        return true;
      }
      return false;
    }, true);
  ...
  _.shuffle = function(array) {
    ...
    for (var i = 0; i < newArr.length; i++) {
      newPos = Math.floor(Math.random() * newArr.length);
      temp = newArr[newPos];
      newArr[newPos] = newArr[i];
      newArr[i] = temp;
    }
    return newArr;
  };
